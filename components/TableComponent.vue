...
const findItemIndex = (target, items) => items.indexOf (items.find (item => item.id == target));

const findItemValue = (target, items) =>
{
    return items?.value[items.value.indexOf (items.value.find (item => item.id == target))];
};
...
const getItems = props.fnApiGetAllItems ?? (async (target) =>
{
    const
...
const setAnItem = async (target) =>
{
    ...
    (success) =>
    {
        datatableBody.items[findItemIndex (success.id, datatableBody.items)] = { ...datatableBody.items[findItemIndex (success.id, datatableBody.items)], ...success, };
...
const fnApiGetItem = props.fnApiGetItem ?? (async (item) =>
{
...
const fnUpdateItem = props.fnUpdateItem ?? (async (item) => {
...
const fnApiExportItems = props.fnApiExportItems ?? (async (extension) =>
{
    const
